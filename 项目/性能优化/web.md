...
- **代码分割**：使用动态 import 按需加载，减少首屏包体积
- **Tree Shaking**：移除未使用的代码，减小打包体积
- **资源压缩**：Gzip/Brotli 压缩，CDN 加速
- **图片优化**：WebP 格式、懒加载、响应式图片
- **字体优化**：字体子集化、preload 关键字体

#### 🚀 渲染性能优化

- **虚拟滚动**：长列表使用虚拟滚动，减少 DOM 节点
- **防抖节流**：高频事件使用防抖/节流，减少函数执行
- **React 优化**：
  - 使用 `React.memo` 避免不必要的重渲染
  - `useMemo`/`useCallback` 缓存计算结果和函数
  - 合理使用 `key` 属性，优化 diff 算法
- **CSS 优化**：避免复杂选择器、减少重排重绘

#### 🚀 网络请求优化

- **请求合并**：合并多个小请求，减少网络往返
- **请求缓存**：合理使用 HTTP 缓存、本地缓存
- **预加载**：关键资源使用 `preload`、`prefetch`
- **接口优化**：减少数据传输量、使用分页/懒加载

#### 🚀 运行时优化

- **Web Workers**：将耗时计算移到 Worker，避免阻塞主线程
- **requestIdleCallback**：利用空闲时间执行非关键任务
- **批量更新**：React 自动批量更新，避免频繁渲染
- **内存管理**：及时清理事件监听、定时器，避免内存泄漏
